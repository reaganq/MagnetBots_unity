...
static public class NGUIMath
{
	...
	static public Bounds CalculateAbsoluteWidgetBounds(Transform trans)
	{
		...
		return CalculateAbsoluteWidgetBounds(widgets);
	}

	static public Bounds CalculateAbsoluteWidgetBounds(IList<UIWidget> widgets)
	{
	...
	static public Bounds CalculateRelativeWidgetBounds (Transform root, Transform child)
	{
		...
		return CalculateRelativeWidgetBounds(root, widgets);
	}

	static public Bounds CalculateRelativeWidgetBounds (Transform root, IList<UIWidget> widgets)
	{
		if (widgets.Count == 0) return new Bounds(Vector3.zero, Vector3.zero);

		Matrix4x4 localFromWorld = root.worldToLocalMatrix;

		// transform from world space to relative-to-parent space
		Bounds b = CalculateAbsoluteWidgetBounds(widgets);
		b.min = localFromWorld.MultiplyPoint3x4(b.min);
		b.max = localFromWorld.MultiplyPoint3x4(b.max);
		return b;
	}
