...
public class UIPanel : MonoBehaviour
{
	...
	public bool IsVisible (UIWidget w)
	{
		...
		if (mClipping == UIDrawCall.Clipping.DeskewedHardClip)
		{
			float skewYAmount = mCam.projectionMatrix.m10 * mCam.projectionMatrix.m00;
			a.y -= a.x * skewYAmount;
			b.y -= b.x * skewYAmount;
		}
	...
	void Start ()
	{
		...
		float scale = (root != null) ? mScreenSize.y / root.activeHeight : 1f;
		mScreenSize /= scale;
	}
	...
	public void UpdateDrawcalls ()
	{
		...
		if (range.z == 0f) range.z = mScreenSize.x * 0.5f;
		if (range.w == 0f) range.w = mScreenSize.y * 0.5f;
	...
	void LateUpdate ()
	{
		...
		UIRoot root = NGUITools.FindInParents<UIRoot>(gameObject);
		if (root != null) mScreenSize *= root.GetPixelSizeAdjustment(Screen.height);
