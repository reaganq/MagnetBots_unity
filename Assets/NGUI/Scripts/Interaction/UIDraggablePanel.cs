...
public class UIDraggablePanel : IgnoreTimeScale
{
	...
	bool shouldMove
	{
		get
		{
			...
			float screenScale = (root != null) ? screenSize.y / root.activeHeight : 1f;
			screenSize /= screenScale;

			float hx = (clip.z == 0f) ? screenSize.x * 0.5f : clip.z * 0.5f;
			float hy = (clip.w == 0f) ? screenSize.y * 0.5f : clip.w * 0.5f;

			if (mPanel.clipping == UIDrawCall.Clipping.SoftClip)
			{
				hx -= mPanel.clipSoftness.x;
				hy -= mPanel.clipSoftness.y;
			}
	...
	void Start ()
	{
		...
		if (horizontalScrollBar != null)
		{
			...
			float alpha;
			if (showScrollBars == ShowCondition.Always) alpha = 1f;
			else if (showScrollBars != ShowCondition.WhenDragging && shouldMoveHorizontally) alpha = 1f;
			else alpha = 0f;
		...
		if (verticalScrollBar != null)
		{
			...
			float alpha;
			if (showScrollBars == ShowCondition.Always) alpha = 1f;
			else if (showScrollBars != ShowCondition.WhenDragging && shouldMoveVertically) alpha = 1f;
			else alpha = 0f;
