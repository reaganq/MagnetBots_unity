...
public class SliderPuzzle : MonoBehaviour {
    ...
    private Hashtable pieceCorrectPositions = new Hashtable();
	...
	protected void Update () {
        ...
        else
        {
            ...
            if (grid!=null && grid.active != showGrid) grid.active = showGrid;
                        ...
                        if (slidingProgress == 1)
                        {
                            ...
                            if (Vector2.Equals(piecePositions[slidingPiece.name],pieceCorrectPositions[slidingPiece.name]))
                            {
                                for (int p=0; p<pieces.Count; p++)
                                {
                                    ...
                                    if (piece.active && piece != slidingPiece)
                                    {
    ...
    private void InitPiece(GameObject puzzlePiece, Vector2 pos)
    {
        ...
        puzzlePiece.renderer.material = main.puzzle;
        ...
        puzzlePiece.renderer.material.mainTextureScale = new Vector2(tsx, tsy);
        // set the right offset in relation to the specified size and piece shrink factor
        puzzlePiece.renderer.material.mainTextureOffset = new Vector2(tsx * ((pos.x - 1) * dsx) + (itsx/2), (tsy * (pos.y * dsy) * -1) + (itsy/2) ); 
    }
